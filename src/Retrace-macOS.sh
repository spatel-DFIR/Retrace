...
# Retrace-macOS.sh
# -----------------------------------------------------------------------------
# Retrace — forensic browser-trail reconstruction for DFIR (macOS). Reads
# Chrome, Edge, Brave, Vivaldi, Chromium, Firefox and Safari history databases
# and produces a record structure (Visits / Downloads / Keywords), time
...
# Usage       : see USAGE-macOS.md, or run with -Help
...
PLATFORM="macOS"
VALID_BROWSERS=(Chrome Edge Firefox Vivaldi Brave Chromium Safari)
...
print_usage() {
cat <<USAGE
Retrace-macOS.sh  v${SCRIPT_VERSION}  by ${SCRIPT_AUTHOR}
macOS browser-history forensic collector (Chrome, Edge, Brave, Vivaldi,
Chromium, Firefox, Safari). Outputs Visits / Downloads / Keywords with
transition decoding, redirect-chain analysis and navigation-chain tracing.

USAGE:
  ./Retrace-macOS.sh [options]

OPTIONS (values are space-separated):
  -Browser <list>        Comma list: Chrome,Edge,Firefox,Vivaldi,Brave,Chromium,Safari,All  (default All)
...
EXAMPLES:
  # Last 24h across all browsers (grant Full Disk Access for Safari)
  ./Retrace-macOS.sh -LastHours 24

  # Chrome + Safari downloads from the last 7 days
  ./Retrace-macOS.sh -Browser Chrome,Safari -Downloads -LastDays 7

  # Investigate an extracted profile or a single database file
  ./Retrace-macOS.sh -Path "/evidence/Default/History" -Browser Chrome
...
parse_args() {
  while [ $# -gt 0 ]; do
    ...
    case "$lk" in
      ...
      -version|--version) printf 'Retrace-macOS.sh v%s by %s\n' "$SCRIPT_VERSION" "$SCRIPT_AUTHOR"; exit 0 ;;
...
# family: chromium|firefox|safari ; col: native timestamp column.
time_where() {
  ...
  case "$family" in
    ...
    safari)   [ -n "$TF_START" ] && s=$(( TF_START - 978307200 ))
              [ -n "$TF_END" ]   && e=$(( TF_END - 978307200 )) ;;
  esac
  [ -n "${s:-}" ] && conds="($col >= $s)"
...
ts_safari()   { printf "strftime('%%Y-%%m-%%d %%H:%%M:%%f', %s + 978307200, 'unixepoch')" "$1"; }
ep_safari()   { printf "(%s + 978307200)" "$1"; }
...
search_where() {   # family qtype  -> SQL fragment or empty
  ...
  case "$family-$qtype" in
    ...
    safari-visits)      printf '(i.url LIKE %s OR v.title LIKE %s)' "$p" "$p" ;;
    safari-keywords)    printf '(i.url LIKE %s)' "$p" ;;
  esac
}
...
SQL
      ...
      ;;
    safari)
      # Safari: history_items (i) + history_visits (v). visit_time = CFAbsoluteTime (REAL secs since 2001).
      local stinfo="CASE WHEN v.redirect_destination IS NOT NULL THEN 'Redirect (source)' WHEN v.redirect_source IS NOT NULL THEN 'Redirect (target)' WHEN v.origin=1 THEN 'Typed/Generated' ELSE 'Link Click' END"
      local stdet="'Type: '||($stinfo)||' | Origin: '||COALESCE(v.origin,'?')||' | Load OK: '||COALESCE(v.load_successful,'?')"
      case "$qtype" in
        visits)
          local w; w=$(join_where "$(search_where safari visits)" "$(time_where safari v.visit_time)")
          local si=$EMPTY sd=$EMPTY
          [ "$INCLUDE_TRANS" = "true" ] && { si="$stinfo"; sd="$stdet"; }
          cat <<SQL
SELECT $(ts_safari v.visit_time), $(ep_safari v.visit_time), $(clean i.url), $(clean v.title),
 $EMPTY, $EMPTY, $si, $sd, $EMPTY, $EMPTY, $EMPTY, $EMPTY,
 v.redirect_source, v.id, v.redirect_destination, $EMPTY, $EMPTY, $EMPTY, $EMPTY, $EMPTY, $EMPTY, $EMPTY, $EMPTY, $EMPTY
FROM history_visits v JOIN history_items i ON v.history_item=i.id
$w ORDER BY v.visit_time DESC;
SQL
          ;;
        keywords)
          local kw="(i.url LIKE '%google.%/search?%q=%' OR i.url LIKE '%bing.com/search?%q=%' OR i.url LIKE '%duckduckgo.com/?%q=%')"
          local w; w=$(join_where "$(search_where safari keywords)" "$(time_where safari v.visit_time)" "$kw")
          cat <<SQL
SELECT $(ts_safari v.visit_time), $(ep_safari v.visit_time), $EMPTY, $(clean v.title), $(clean i.url),
 $EMPTY, $EMPTY, $EMPTY, $EMPTY, $EMPTY, $EMPTY, $EMPTY, $EMPTY, v.id, $EMPTY, $EMPTY,
 $EMPTY, $EMPTY, $EMPTY, $EMPTY, $EMPTY, $EMPTY, $EMPTY, $EMPTY
FROM history_visits v JOIN history_items i ON v.history_item=i.id
$w ORDER BY v.visit_time DESC;
SQL
  ...
  if ! err=$(cp -p "$orig" "$dst" 2>&1); then
    ...
    case "$err" in
      *"Operation not permitted"*|*"Permission denied"*)
        case "$orig" in
          *"/Library/Safari/"*)
            warn "  -> Safari history is protected by macOS. Grant Full Disk Access to your terminal:"
            warn "     System Settings > Privacy & Security > Full Disk Access > enable Terminal/iTerm, then re-run. (sudo does NOT bypass this.)" ;;
          *)
            warn "  -> Permission denied. Grant Full Disk Access to your terminal, or run as the profile owner / via sudo for other users." ;;
        esac ;;
    esac
    return 1
  fi
...
browser_base() {   # home browser -> base dir (chromium 'user data' root or firefox profiles root or safari file)
  local home="$1" b="$2"
  case "$b" in
    ...
    Chromium) echo "$home/Library/Application Support/Chromium" ;;
...
discover_for_home() {
  ...
  for b in "${SELECTED[@]}"; do
    ...
    if [ "$b" = "Firefox" ]; then
      ...
      [ -f "$base/History.db" ] && DISCOVERED+="$user$SEP$b$SEP$base/History.db"$'\n'
    else
      [ -d "$base" ] || continue
      local pf name
      while IFS= read -r pf; do
...
discover_custom() {
  ...
  local chromium_sel=0 firefox_sel=0 safari_sel=0 b
  for b in "${SELECTED[@]}"; do
    case "$b" in Firefox) firefox_sel=1 ;; Safari) safari_sel=1 ;; *) chromium_sel=$((chromium_sel+1)) ;; esac
  done
  local files=()
  if [ -f "$cp" ]; then
    files=("$cp")
  else
    while IFS= read -r f; do files+=("$f"); done < <(find "$cp" \( -name History -o -name places.sqlite -o -name History.db \) -type f 2>/dev/null)
  fi
  local f low brow
  for f in "${files[@]}"; do
    ...
    if [ "$bn" = "History.db" ]; then
      [ "$safari_sel" -eq 1 ] && DISCOVERED+="$leaf${SEP}Safari$SEP$f"$'\n'
      continue
    fi
...
# Enumerate target users/homes (mac: /Users)
enumerate_targets() {
  if [ -n "$PATH_ARG" ]; then
  ...
  if [ -n "$USERNAME" ]; then
    ...
    case "$USERNAME" in
      ...
      */*|*..*) die "-UserName must be a bare account name or an absolute path" ;;
      *)  home="/Users/$USERNAME"
          [ -d "$home" ] || home=$(dscl . -read "/Users/$USERNAME" NFSHomeDirectory 2>/dev/null | awk '{print $2}') ;;
    esac
    [ -d "$home" ] || { warn "User home not found for '$USERNAME'"; return; }
    discover_for_home "$(basename "$home")" "$home"; return
  fi
  if [ "$(id -u)" -eq 0 ]; then
    vlog "Running as root: scanning all /Users profiles"
    local d name
    while IFS= read -r d; do
      name=$(basename "$d")
      case "$name" in Shared|.localized|Guest) continue ;; esac
...
db_summary() {
  ...
  case "$family" in
    ...
    safari)  table=history_visits;    datecol=visit_time; econv="+978307200" ;;
...
analyze_cleaning() {
  ...
  case "$family" in
    ...
    safari)  table=history_visits; datecol=visit_time; econv="+978307200"; seqname=history_visits
             urlmm_sql="SELECT COUNT(*) FROM history_items i WHERE i.visit_count > (SELECT COUNT(*) FROM history_visits v WHERE v.history_item=i.id);" ;;
...
# Pull http(s) URLs out of a Safari .plist (converted to XML via plutil, read-only).
# Drops the boilerplate plist DOCTYPE URL so it is not counted as an artifact.
plist_urls() {
  [ -f "$1" ] || return 0
  command -v plutil >/dev/null 2>&1 || return 0
  plutil -convert xml1 -o - "$1" 2>/dev/null | grep -oE 'https?://[^<]+' | grep -v 'apple.com/DTDs/PropertyList'
}
...
# tabs) are the strong signal; contact/embedded artifacts (cookies, NEL, storage)
# also include third-party domains. Also reports incognito posture and lists
...
collect_private() {
  ...
  elif [ "$family" = "safari" ]; then
    sql_query "$histtmp" "SELECT url FROM history_items" | normalize_hosts | sort -u > "$HISTHOSTS"
    {
      sqlite_artifact 'iCloud tab URLs' "$profdir/CloudTabs.db" 'SELECT url FROM cloud_tabs'
      plist_urls "$profdir/TopSites.plist"           | report_orphan_hosts 'Top Sites'
      plist_urls "$profdir/Bookmarks.plist"          | report_orphan_hosts 'Bookmarked domains'
      plist_urls "$profdir/LastSession.plist"        | report_orphan_hosts 'Last-session tabs'
      plist_urls "$profdir/RecentlyClosedTabs.plist" | report_orphan_hosts 'Recently-closed tabs'
      printf '  %-23s : not determinable from Safari config (no incognito posture file)\n' 'Private posture'
      present_binary 'Present (not parsed)' "$(dirname "$profdir")/Cookies/Cookies.binarycookies" "$profdir/Favicon Cache"
    } >> "$bodyfile"
  fi
...
follow_chain() {
  ...
  while IFS="$SEP" read -r user browser db; do
    ...
    [ "$family" = "safari" ] && continue   # Safari visit ids differ; chain handled via generic analysis
...
main() {
  ...
  if [ -s "$PRIVATE_DETAIL" ]; then
    ...
    log "bookmark, tab, storage, ...) but has NO matching history visit was reached"
    log "yet never recorded - private OR deleted. First-party artifacts are the"
    log "strong signal; cookies/NEL/storage also include third-party domains. OS-"
    log "level residue (DNS, prefetch, memory) and binary formats (sessions, cache,"
    log "binarycookies) are out of scope / listed but not parsed."
    log ""
...
# -----------------------------------------------------------------------------
# Safari downloads (stored in ~/Library/Safari/Downloads.plist, not SQLite).
# Parsed read-only with plutil (native) -> XML on stdout (no temp file written).
# -----------------------------------------------------------------------------
# Un-escape the XML entities plutil emits inside <string>/<date> values.
xml_unescape() {
  printf '%s' "$1" | sed 's/&lt;/</g; s/&gt;/>/g; s/&quot;/"/g; s/&#39;/'"'"'/g; s/&apos;/'"'"'/g; s/&amp;/\&/g'
}

safari_downloads() {
  local user="$1" browser="$2" db="$3"
  local plist; plist="$(dirname "$db")/Downloads.plist"
  [ -f "$plist" ] || return 0
  command -v plutil >/dev/null 2>&1 || { warn "plutil not found; skipping Safari downloads"; return 0; }
  local TAB; TAB=$(printf '\t')
  # Extract (url, path, isodate) triples (one per dict), then convert/filter in
  # bash so the time filter applies and XML entities are decoded.
  plutil -convert xml1 -o - "$plist" 2>/dev/null | awk '
    function emit(){ if(url!="" || path!=""){ printf "%s\t%s\t%s\n", url, path, dt; url="";path="";dt="" } }
    /<key>DownloadEntryURL<\/key>/        { getline; gsub(/^[ \t]*<string>|<\/string>[ \t]*$/,""); url=$0 }
    /<key>DownloadEntryPath<\/key>/       { getline; gsub(/^[ \t]*<string>|<\/string>[ \t]*$/,""); path=$0 }
    /<key>DownloadEntryDateAddedKey<\/key>/ { getline; gsub(/^[ \t]*<date>|<\/date>[ \t]*$/,""); dt=$0 }
    /<\/dict>/                            { emit() }
  ' | while IFS="$TAB" read -r url path dt; do
      url=$(xml_unescape "$url"); path=$(xml_unescape "$path")
      local ep="" tsfmt=""
      if [ -n "$dt" ]; then
        local clean; clean=$(printf '%s' "$dt" | tr 'T' ' ' | tr -d 'Z')
        ep=$(to_epoch "$clean"); tsfmt="$clean.000"
      fi
      # Honor the time filter (these are NOT SQL records, so filter here).
      if [ "$TF_ACTIVE" -eq 1 ]; then
        [ -n "$ep" ] || continue
        [ -n "$TF_START" ] && [ "$ep" -lt "$TF_START" ] && continue
        [ -n "$TF_END" ] && [ "$ep" -ge "$TF_END" ] && continue
      fi
      # Build a 27-field record: user,browser,Download + 24 data cols
      # (d1 ts, d2 epoch, d3 url, d4 title='', d5 searchurl='', d6 path, d7..d24='')
      local line="$user$SEP$browser${SEP}Download$SEP$tsfmt$SEP${ep:-0}$SEP$url$SEP$SEP$SEP$path"
      local i; for i in $(seq 7 24); do line="$line$SEP"; done
      printf '%s\n' "$line"
    done
}
